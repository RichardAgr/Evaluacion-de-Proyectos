...
function SeguimientoSemanalSprints () {
    ...
    const navigateSemana=(idSprint, idSemana, sprintIndex, semanaIndex)=>{
        const hasPendingWeeks = verificacion
        .slice(0, sprintIndex + 1) // Incluye el sprint actual
        .some((sprint, i) => 
            i < sprintIndex // Para sprints anteriores, revisa todas las semanas
                ? !sprint.completo
                : !sprint.completoSemanas.slice(0, semanaIndex).every(Boolean) // Para el sprint actual, revisa semanas previas
        );

        if (hasPendingWeeks) {
            setSnackbar({
                open: true,
                message: "Primero debes hacer el seguimiento de las anteriores Semanas a esta",
                severity: "info",
                autoHide: 6000,
            });
            return;
